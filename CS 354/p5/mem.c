...
void* Mem_Alloc(int size)
{
  ...
  // Traverse list, keeping track of the smallest block large enough to be used
  while(current != NULL) {
    if(current->size_status % 4 == 0 && current->size_status >= size) {
      if(best == NULL) {
	best = current;
      }
      else if(current->size_status < best->size_status) {
	best = current;
      }
    }
  ...
  // If a block was found to allocate, split if neccessary
  if(best != NULL) {
    int split = best->size_status - size -  12;
    if(split >= 0) {
      void* new_header = (char *) best + size + sizeof(block_header);
      block_header *new_block = (block_header *) (new_header);
      new_block->size_status = split + 4;
...
int Mem_Free(void *ptr)
{
  ...
  // Move pointer to point to header
  ptr -= sizeof(block_header);

  block_header* current = list_head;
  block_header* prev = NULL;

  // Traverse List checking if ptr points to an allocated block
  while(current != NULL) {
    if(ptr == current) {
      if(current->size_status % 4 != 1) {
	break;
      }
      // Mark Block as free
      current->size_status--;
      
      // Coaleasce when possible
  	
      if(prev != NULL) {
	if(prev->size_status % 4 == 0) {
	  prev->next = current->next;
	  prev->size_status += current->size_status + 8;
	  current = prev;
			}
	}
      if(current->next != NULL) {
	if(current->next->size_status % 4 == 0) {
	  current->size_status += current->next->size_status + 8;
